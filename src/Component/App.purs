...
contractCashFlowInfo (Just (BlockHeader { slotNo: SlotNumber slot })) contractTerms party counterParty (Just (MarloweInfo { state })) possibleUserContractRole transactions = do
  let
    ...
    cashFlows'' = Array.zipWith { tx: _, cf: _ } transactions' (Array.fromFoldable cashFlows')

  Array.catMaybes $
    map
      ( \{ cf, tx } -> do
          cashFlow@(Actus.CashFlow { currency, amount }) <- cf
          let
...
type CashFlow' = CashFlow Value' V1.Party

addChoiceMarker :: List (Maybe CashFlow') -> List (Maybe CashFlow')
...
hasRiskFactor :: CashFlow Value' V1.Party -> Boolean
